package PostScript::PStools::PS2png;
...
PostScript::PStools::PS2png - Convert PS to PNG using Ghostscript
...
the web interface at L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=PostScript-PStools-PS2png>.  I will be notified, and then you'll
...
L<http://rt.cpan.org/NoAuth/Bugs.html?Dist=PostScript-PStools-PS2png>

=item * AnnoCPAN: Annotated CPAN documentation

L<http://annocpan.org/dist/PostScript-PStools-PS2png>

=item * CPAN Ratings

L<http://cpanratings.perl.org/d/PostScript-PStools-PS2png>

=item * Search CPAN

L<http://search.cpan.org/dist/PostScript-PStools-PS2png/>
...
sub ps2png {
	...
	# size in points (1/72 inch) of input file
	...
	my $x_sf;
	if ($rotate) {
		# assuming y > x in landscape mode
	...
	# start with x scaling factor, ; adjust aspect ratio if specified
	# assume we're reducing size, so divide rather than multiply
	...
	my $y_out_px;
	if ($rotate) {
		# if rotating, we need to pivot the y dimension to be
		# less than x, so scale it by the input aspect ratio
		$y_out_pt = sprintf "%.2f",$y_in_pt * $y_sf * $in_aspect;
		$y_out_px = sprintf "%.0f", $y_out_pt * (300 / 72);
		# now we have to swap the x and y scaling factors
		($x_sf,$y_sf) = ($y_sf,$x_sf);
	} else {
	...
	if ($rotate) {
		$rotation = 3;
		# and rotation means we need to swap x and y
		# canvas dimensions
	...
	my $rotation_string = 
		" -c '<</Orientation " .  $rotation . ">> setpagedevice'";
	...
	if ($verbose) {
		...
		print "points: x= $x_in_pt y = $y_in_pt\n";
		print "pixels: x= $x_in_px y = $y_in_px\n";
		print "inches: x= $x_in_in y = $y_in_in\n";
		print "\n";
		print "input file aspect ratio= $in_aspect\n";
		print "user-specified aspect adjustment= $aspect\n";
		print "scaling factors x= $x_sf, y= $y_sf\n";
		print "\n";
		print "Output image size: \n";
		print "points: x=  $x_out_pt y= $y_out_pt\n";
		print "pixels: x= $x_out_px y= $y_out_px\n";
	...
	my $gscmd = "gs -q -sDEVICE=pnggray -dSAFER -dNOPAUSE -dBATCH " . 
				" -r300 -dGraphicsAlphaBits=4 -dTextAlphaBits=4" .
				" -dDEVICEWIDTHPOINTS=$device_width_points" .
				" -dDEVICEHEIGHTPOINTS=$device_height_points" .
				" -sOutputFile=" . $outfile .
				$rotation_string .
				 " - ";
...
1; # End of PostScript::PStools::PS2png
